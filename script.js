...
// === Dynamic navbar offset (prevents content from sitting under fixed nav)
(function initNavOffset() {
    ...
    // Run after full load so computed height is accurate (fonts/icons applied)
    window.addEventListener('load', setNavOffset);

    // Keep it accurate on viewport changes
    window.addEventListener('resize', setNavOffset);

    // React to navbar’s own size changes (breakpoints, content changes)
    const nav = document.getElementById('appNavbar');
    if (window.ResizeObserver && nav) {
        new ResizeObserver(setNavOffset).observe(nav);
    }

    // Early pass to reduce initial underlap during first paint
