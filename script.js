...
if (cloudBtn && cloudMenu) {
    // NEW: apply professional visual classes & aria-controls without touching HTML
    cloudBtn.classList.add('cloud-btn');                // visual polish
    cloudBtn.setAttribute('aria-controls', 'cloudSyncMenu');
    const badgeEl = document.getElementById('cloudStatusBadge');
    if (badgeEl) badgeEl.classList.add('cloud-badge');  // visual polish
    ...
    // NEW: keyboard activation for reliability & a11y
...
function render() {
    ...
    if (itemBreakdown) {
        ...
        Object.entries(itemCounts).forEach(([itemName, count]) => {
            const div = document.createElement('div');
            // Compact vertical rhythm for each row
            div.className = 'flex items-baseline justify-between py-0.5 md:py-1';
        ...
        // Footer row: Total pieces — reduced gap to match tables
        const totalPieces = Object.values(itemCounts).reduce((sum, n) => sum + n, 0);
        const footer = document.createElement('div');
        footer.className = 'mt-2 flex items-baseline justify-between border-t pt-2';
        footer.innerHTML = `
...
function renderTransactions() {
    ...
    for (const t of all) {
        ...
        // match CSS class names that style the colored pills
        const typeClass = t.type === 'payment' ? 'pill pill--pay' : 'pill pill--ded';
        const notesEsc = (t.notes || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

        // Escape method text to avoid HTML injection
        const methodEsc = (t.method || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

        tr.innerHTML = `
    <td class="py-2 pr-3 whitespace-nowrap">${fmtDate(t.date)}</td>
    <td class="py-2 pr-3"><span class="${typeClass}">${typeLabel}</span></td>
    <td class="py-2 pr-3">${methodEsc}</td> <!-- NEW: Method -->
    <td class="py-2 pr-3 text-right tabular-nums">${fmt(t.amountCents)}</td>
    <td class="py-2 pr-3 max-w-[360px] truncate">${notesEsc}</td>
...
`;
        ...
        if (isManual) {
            ...
            if (btn) {
                btn.addEventListener('click', () => {
                    // Use shared confirm modal (no browser popup)
                    showConfirmModal(
                        'Are you sure you want to delete this transaction?',
                        () => { deleteTransaction(t.id); },       // same logic as before
                        'Delete Transaction',
                        'danger'
                    );
                });
