...
document.addEventListener('click', (e) => {
    ...
    // If click target is not inside the panel or the button, hide the panel
...
/* =========================
 * Cloud Sync (Google Drive AppData) - Offline-first automatic backup
 * Minimal, self-contained, and does NOT change existing logic.
 * Requires a Google OAuth Web Client ID: replace GOOGLE_CLIENT_ID below.
 * ========================= */
(function () {
    const GOOGLE_CLIENT_ID = window.BT_GOOGLE_CLIENT_ID || "4853441079-chf1qiv8a06pfjk6a2c1b6eq4uosi765.apps.googleusercontent.com";
    ...
    const STATE = {
        ...
        signedIn: false,
        latestFileId: null,
        status: "Needs sign-in" // "Up to date" | "Syncing…" | "Offline" | "Needs sign-in"
    };
    ...
    // Local snapshot: capture all Bakery Tracker keys 1:1
    function buildSnapshot() {
        const payload = { localStorage: {} };
        for (let i = 0; i < localStorage.length; i++) {
            const k = localStorage.key(i);
            // Only sync Bakery Tracker keys to be safe
            if (k && (k.startsWith("bakery-tracker-") || k === "darkMode")) {
        ...
        return {
            ...
            deviceId: (localStorage.getItem("bakery-tracker-device-id") || (function () {
                const id = Math.random().toString(36).slice(2) + Date.now().toString(36);
                localStorage.setItem("bakery-tracker-device-id", id);
                return id;
            })()),
            payload
        };
    ...
    function applySnapshot(snap) {
        if (!snap || !snap.payload || !snap.payload.localStorage) return;
        const data = snap.payload.localStorage;
    ...
    function renderStatusChip(el, status) {
        const map = {
            ...
            "Needs sign-in": "bg-neutral-200 text-neutral-700"
        };
        el.textContent = status;
        el.className = "px-2 py-0.5 rounded text-[11px] " + (map[status] || "bg-neutral-200 text-neutral-700");
    }
    ...
    function buildUi() {
        ...
        wrap.id = "btCloudSyncWrap";
        wrap.className = "no-print relative flex items-center gap-2";

        // Button
        const btn = document.createElement("button");
        btn.id = "btCloudBtn";
        btn.className = "px-3 py-1 rounded-md border text-sm transition-base hover:bg-neutral-50 flex items-center gap-2";
        btn.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i><span>Sync</span>';
        wrap.appendChild(btn);

        // Status chip
        const chip = document.createElement("span");
        chip.id = "btCloudStatus";
        renderStatusChip(chip, STATE.status);
        wrap.appendChild(chip);

        // Menu
        const menu = document.createElement("div");
        menu.id = "btCloudMenu";
        menu.className = "absolute right-0 top-full mt-2 w-48 rounded-xl border bg-white shadow-lg p-2 hidden z-40";
        menu.innerHTML = [
            '<button class="w-full text-left px-3 py-2 rounded hover:bg-neutral-50 text-sm" id="btCloudSignIn"><i class="fa-solid fa-right-to-bracket mr-2"></i>Sign in</button>',
            '<button class="w-full text-left px-3 py-2 rounded hover:bg-neutral-50 text-sm" id="btCloudSyncNow"><i class="fa-solid fa-rotate mr-2"></i>Sync now</button>',
            '<button class="w-full text-left px-3 py-2 rounded hover:bg-neutral-50 text-sm" id="btCloudRestore"><i class="fa-solid fa-cloud-arrow-down mr-2"></i>Restore from cloud</button>',
            '<button class="w-full text-left px-3 py-2 rounded hover:bg-neutral-50 text-sm" id="btCloudSignOut"><i class="fa-solid fa-right-from-bracket mr-2"></i>Sign out</button>'
        ].join("");
        wrap.appendChild(menu);

        // Insert before dark toggle (right side of header)
        ...
        // Buttons
        $("btCloudSignIn").addEventListener("click", async () => {
            menu.classList.add("hidden");
            await signIn();
        });
        $("btCloudSignOut").addEventListener("click", async () => {
            menu.classList.add("hidden");
            await signOut();
        });
        $("btCloudSyncNow").addEventListener("click", async () => {
            menu.classList.add("hidden");
            await syncNow();
        });
        $("btCloudRestore").addEventListener("click", async () => {
            menu.classList.add("hidden");
    ...
    function setStatus(s) {
        STATE.status = s;
        const chip = document.getElementById("btCloudStatus");
        if (chip) renderStatusChip(chip, s);
    }
    ...
    function initTokenClient() {
        ...
        STATE.tokenClient = google.accounts.oauth2.initTokenClient({
            ...
            scope: DRIVE_SCOPE,
            prompt: "", // no forced consent after first grant
            callback: (resp) => {
                if (resp && resp.access_token) {
                    STATE.token = resp.access_token;
                    // tokens are typically 3600s; we set expiry conservatively
                    STATE.tokenExp = Date.now() + 55 * 60 * 1000;
                    STATE.signedIn = true;
    ...
    function ensureToken() {
        return new Promise((resolve, reject) => {
            if (STATE.token && Date.now() < STATE.tokenExp) return resolve(STATE.token);
            if (!STATE.tokenClient) {
                if (!initTokenClient()) {
                    return reject(new Error("Google Identity Services not ready. Check client ID or network."));
                }
            }
            try {
                STATE.tokenClient.requestAccessToken({ prompt: STATE.signedIn ? "" : "consent" });
                // Wait briefly; callback above sets token
                let tries = 0;
                const t = setInterval(() => {
                    tries++;
                    if (STATE.token) { clearInterval(t); resolve(STATE.token); }
                    else if (tries > 50) { clearInterval(t); reject(new Error("Failed to obtain token")); }
                }, 100);
            } catch (e) {
                reject(e);
            }
    ...
    // Drive helpers
    async function driveList(q, fields = "files(id,name,modifiedTime,size)", orderBy = "modifiedTime desc") {
    ...
    async function driveFindByName(name) {
        const data = await driveList(`name='${name.replace(/'/g, "\\'")}' and 'appDataFolder' in parents`);
        return data.files && data.files[0] ? data.files[0] : null;
    }

    async function driveCreate(name, jsonObj) {
        const token = await ensureToken();
        const meta = { name, parents: ['appDataFolder'] };
        const boundary = "xxxxxxxxxx" + Math.random().toString(36).slice(2);
        const body =
            `--${boundary}\r\n` +
    ...
    async function driveUpdate(fileId, jsonObj) {
        const token = await ensureToken();
        const r = await fetch("https://www.googleapis.com/upload/drive/v3/files/" + encodeURIComponent(fileId) + "?uploadType=media&fields=id,name", {
            method: "PATCH",
            headers: {
                Authorization: "Bearer " + token,
                "Content-Type": "application/json; charset=UTF-8"
    ...
    async function signIn() {
        ...
        if (!ok) {
            showToast && showToast("Google services not ready. Try again in a moment.", "error");
            return;
        }
        try {
            await ensureToken();
            ...
            showToast && showToast("Sign-in failed.", "error");
        }
    }
    ...
    async function signOut() {
        ...
        setStatus("Needs sign-in");
        showToast && showToast("Signed out of Cloud Sync.", "info");
    }

    async function syncNow() {
        if (!STATE.signedIn) {
            // Attempt token silently
            try { await ensureToken(); STATE.signedIn = true; } catch { setStatus("Needs sign-in"); return; }
        }
        STATE.syncing = true;
        setStatus("Syncing…");
        try {
            const snap = buildSnapshot();
            // Update/Upsert current.json
            const CURRENT = "bakery-tracker-current.json";
            let f = await driveFindByName(CURRENT);
            if (f) {
                await driveUpdate(f.id, snap);
            } else {
                f = await driveCreate(CURRENT, snap);
            }
            // Also create a timestamped snapshot (keep last 5)
            const tsName = "bakery-tracker-" + snap.updatedAt.replace(/[:.]/g, "-") + ".json";
            await driveCreate(tsName, snap);
            // prune older
            try {
                const listed = await driveList(`'appDataFolder' in parents and name contains 'bakery-tracker-'`);
                const ids = (listed.files || []).slice(5).map(f => f.id);
                await Promise.all(ids.map(id => fetch("https://www.googleapis.com/drive/v3/files/" + id, {
                    method: "DELETE", headers: { Authorization: "Bearer " + STATE.token }
                })));
            } catch (_) { }
            setStatus("Up to date");
            showToast && showToast("Cloud sync complete.", "success");
        } catch (e) {
            console.error(e);
            setStatus(navigator.onLine ? "Needs sign-in" : "Offline");
            showToast && showToast("Cloud sync failed.", "error");
        } finally {
            STATE.syncing = false;
        }
    }
    ...
    async function restoreFromCloud() {
        try {
            const latest = await driveFindByName("bakery-tracker-current.json");
            if (!latest) { showToast && showToast("No cloud backup found.", "info"); return; }
            const snap = await driveDownload(latest.id);
            if (!snap || !snap.payload) { showToast && showToast("Backup file invalid.", "error"); return; }
            showConfirmModal && showConfirmModal("Restore from cloud? This will overwrite local data.", () => {
                applySnapshot(snap);
                // Easiest reliable refresh
                location.reload();
            }, "Restore from cloud");
        } catch (e) {
            console.error(e);
            showToast && showToast("Restore failed.", "error");
        }
    }

    // Auto-sync on changes: monkey-patch localStorage.setItem
    const _set = localStorage.setItem.bind(localStorage);
    const scheduleSync = debounce(() => { if (STATE.signedIn) syncNow(); }, 5000);
    ...
    window.addEventListener("focus", () => { if (STATE.signedIn) scheduleSync(); });

    // Expose for menu handlers
    window.btCloud = { signIn, signOut, syncNow, restoreFromCloud };
