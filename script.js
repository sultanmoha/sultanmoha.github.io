...
//
// The legacy app includes a cloud sync button that opens a menu of actions
// (sign in, sync now, restore, view backups, set passphrase, sign out). In
// earlier versions this menu was shown via markup in the old navbar and
// controlled by an external script. After migrating all toolbar functions to
// the new navbar, we must explicitly wire up the toggle behaviour here so
// clicking the button will reveal the menu. Without this handler the menu
// would remain hidden because there is no default handler registered. The
// implementation below simply toggles the `hidden` class on the menu and
// updates the `aria-expanded` attribute. It also listens for outside clicks
// and the Escape key to close the menu gracefully. This ensures parity with
// the behaviour users expect from the legacy interface.
