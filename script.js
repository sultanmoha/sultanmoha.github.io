...
// Prefer the table's <tbody>; fall back to the old div id for safety
const transactionsListEl =
    document.getElementById('transactionsTbody') || $('transactionsList');
...
const transMethodEl = $('transMethod'); // NEW
...
// Grab the custom nav dark icon (sun/moon) button. It will reflect the current theme.
...
// Keep the nav dark icon (sun/moon) in sync with the current theme. When in
// dark mode, display a moon; otherwise display a sun. This function runs
// immediately on load and whenever the dark mode is toggled.
function updateNavDarkIcon() {
    if (!navDarkIcon) return;
    const isDarkMode = rootEl.classList.contains('dark');
    if (isDarkMode) {
        navDarkIcon.classList.remove('fa-sun');
        navDarkIcon.classList.add('fa-moon');
    } else {
        navDarkIcon.classList.remove('fa-moon');
        navDarkIcon.classList.add('fa-sun');
    }
}
...
function showSection(sectionId) {
    ...
    function activate(btn, active) {
        ...
        btn.classList.remove('bg-neutral-200', 'text-neutral-900', 'bg-blue-600', 'text-white');
        // Apply new active colouring on selection
        if (active) {
            btn.classList.add('bg-blue-600', 'text-white');
        }
        btn.setAttribute('aria-selected', active ? 'true' : 'false');
        // Retain nav-tab--active toggle for potential CSS hooks
...
/**
 * Populate the dropdown with Saves only (no shops).
 * - Top option: "All Saved"
 * - One option per active (non-deleted) save
 * - Value format: `save:<index>`; Label: save name only (no timestamps)
 */
function populateShopDropdown() {
    const dropdown = document.getElementById('shopDropdown');
    if (!dropdown) return;

    const prev = String(dropdown.value || '');

    // Ensure saves are available
    if (typeof loadSaves === 'function' && (!Array.isArray(saves) || !saves.length)) {
        try { loadSaves(); } catch { /* ignore */ }
    }
    const activeSaves = Array.isArray(saves)
        ? saves.map((entry, index) => ({ entry, index }))
            .filter(x => x.entry && !x.entry.deleted)
        : [];

    // Reset options
    dropdown.innerHTML = '';

    // All Saved (clears specific save selection)
    dropdown.appendChild(new Option('All Saved', ''));

    // Individual saves (label = name only)
    activeSaves.forEach(({ entry, index }) => {
        const label = (entry.name && entry.name.trim()) ? entry.name.trim() : `Save ${index + 1}`;
        dropdown.appendChild(new Option(label, `save:${index}`));
    });

    // Preserve prior selection if still valid
    const valid = new Set(['', ...activeSaves.map(x => `save:${x.index}`)]);
    dropdown.value = valid.has(prev) ? prev : '';
}

// Inline edit: click any editable table cell to edit in place (Deliveries)
...
// Inline edit: Transactions — delegate to tbody for manual entries
const transactionsTbody = document.getElementById('transactionsTbody');
if (transactionsTbody && !transactionsTbody.dataset.hasEditorHandler) {
    transactionsTbody.dataset.hasEditorHandler = 'true';
    transactionsTbody.addEventListener('click', (ev) => {
        const td = ev.target.closest('td.editable.txn-edit');
        if (td) startTransactionEdit(td);
    });
}
...
// Set up saves-only dropdown (no shops)
const shopDropdownEl = document.getElementById('shopDropdown');
if (shopDropdownEl) {
    // Populate options from saves (with "All Saved")
    populateShopDropdown();

    shopDropdownEl.addEventListener('change', (ev) => {
        const val = String(ev.target.value || '');

        // Selecting an individual Save replaces current data and re-renders
        if (val.startsWith('save:')) {
            const idx = parseInt(val.slice(5), 10);
            if (!isNaN(idx)) {
                restoreSave(idx, 'replace'); // restoreSave -> save()/render() pipeline
                // Rebuild and keep UI in sync with the chosen save
                populateShopDropdown();
                const dd = document.getElementById('shopDropdown');
                if (dd) dd.value = `save:${idx}`;
            }
            return;
        }

        // "All Saved": no specific save selected — just render current dataset
        currentShopFilter = null; // ensure no shop scoping remains
        render();
    });
}
...
/* ========================= Transactions inline editing ========================= */

/**
 * Start editing a manual transaction cell (date, type, method, amountCents, notes).
 * Auto-generated "Paid" rows are read-only and ignored here.
 * @param {HTMLTableCellElement} td
 */
function startTransactionEdit(td) {
    const tr = td.closest('tr');
    const id = tr?.dataset?.tid;
    const field = td.dataset.field;
    if (!id || !field) return;
    if (String(id).startsWith('auto-')) return; // not editable
    ...
    if (field === 'date') {
        ...
        input.type = 'date';
        input.value = (t.date && /\d{4}-\d{2}-\d{2}/.test(t.date))
            ? t.date
            : new Date(t.date).toISOString().slice(0, 10);
    } else if (field === 'type') {
        input = document.createElement('select');
        [['payment', 'Payment'], ['deduction', 'Deduction']].forEach(([val, label]) => {
            const opt = document.createElement('option');
            opt.value = val; opt.textContent = label; input.appendChild(opt);
        });
        input.value = (t.type === 'deduction') ? 'deduction' : 'payment';
    } else if (field === 'method') {
        input = document.createElement('select');
        const methods = ['Adeeg', 'Cash', 'Check', 'Zelle', 'Card', 'Other'];
        ...
        // If existing value isn’t in our list, keep it selectable
        if (t.method && !methods.includes(t.method)) {
            const opt = document.createElement('option');
            opt.value = t.method;
            opt.textContent = t.method;
            input.appendChild(opt);
        }

        input.value = t.method || '';
    } else if (field === 'amountCents') {
        input = document.createElement('input');
        input.type = 'number';
        input.step = '0.01';
        input.min = '0';
        input.value = ((t.amountCents || 0) / 100).toFixed(2);
    } else if (field === 'notes') {
        input = document.createElement('input');
        input.type = 'text';
        input.value = t.notes || '';
    ...
    const commit = () => { applyTransactionUpdate(id, field, input.value); };
    const cancel = () => { td.innerHTML = original; };
...
/**
 * Apply an update to a manual transaction and refresh UI.
 * @param {string} id
 * @param {string} field
 * @param {string} rawValue
 */
function applyTransactionUpdate(id, field, rawValue) {
    const t = transactions.find(x => x.id === id);
    if (!t) return;

    switch (field) {
        case 'date':
            t.date = rawValue || t.date;
            break;
        case 'type':
            t.type = (rawValue === 'deduction') ? 'deduction' : 'payment';
            break;
        case 'method':
            t.method = String(rawValue || '').trim();
            break;
        case 'amountCents':
            t.amountCents = parseCents(rawValue);
            break;
        case 'notes':
            t.notes = String(rawValue || '');
            break;
    }
    saveTransactions();
    render();
...
// "More" menu button toggles additional export/import actions
const navMore = document.getElementById('navMore');
const moreMenu = document.getElementById('moreMenu');
if (navMore && moreMenu) {
    navMore.addEventListener('click', () => {
        const expanded = navMore.getAttribute('aria-expanded') === 'true';
        if (expanded) {
            moreMenu.classList.add('hidden');
            navMore.setAttribute('aria-expanded', 'false');
        } else {
            // close any other open menus (e.g., savePanel) to avoid overlaps
    ...
    document.addEventListener('click', (e) => {
        ...
        if (navMore.contains(e.target) || moreMenu.contains(e.target)) return;
        moreMenu.classList.add('hidden');
        navMore.setAttribute('aria-expanded', 'false');
    });
    // Close on escape key
...
// -----------------------------------------------------------------------------
// Cloud Sync button toggle for new navbar
const cloudBtn = document.getElementById('cloudSyncBtn');
const cloudMenu = document.getElementById('cloudSyncMenu');
if (cloudBtn && cloudMenu) {
    // Ensure ARIA wiring only; do NOT force circular button styling
    // Remove legacy 'cloud-btn' if present so it matches other icons
    cloudBtn.setAttribute('aria-controls', 'cloudSyncMenu');
    cloudBtn.classList.remove('cloud-btn');

    const badgeEl = document.getElementById('cloudStatusBadge');
    if (badgeEl) badgeEl.classList.add('cloud-badge'); // keep badge visuals

    // Close any other open panels (More, Saves, Purchases Save) before opening Cloud
    function closeOtherPanels(exceptEl) {
        const panels = document.querySelectorAll('.menu-panel');
        panels.forEach(p => { if (p !== exceptEl) p.classList.add('hidden'); });

        // Reset ARIA on known triggers
        const navMore = document.getElementById('navMore');
        if (navMore) navMore.setAttribute('aria-expanded', 'false');
        const navSave = document.getElementById('navSave');
        if (navSave) navSave.setAttribute('aria-expanded', 'false');
    }

    const toggleCloudMenu = () => {
        const expanded = cloudBtn.getAttribute('aria-expanded') === 'true';
        if (expanded) {
            cloudMenu.classList.add('hidden');
            cloudBtn.setAttribute('aria-expanded', 'false');
        } else {
            closeOtherPanels(cloudMenu); // NEW: close others on open
            cloudMenu.classList.remove('hidden');
            cloudBtn.setAttribute('aria-expanded', 'true');
            // Focus the first interactive element in the menu for accessibility
            const first = cloudMenu.querySelector('button, input, [tabindex]:not([tabindex="-1"])');
            if (first && first.focus) setTimeout(() => first.focus(), 10);
        }
    };
    ...
    // Keyboard activation for reliability & a11y
    cloudBtn.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            const willOpen = cloudBtn.getAttribute('aria-expanded') !== 'true';
            if (willOpen) {
                closeOtherPanels(cloudMenu); // NEW: close others on open
                cloudMenu.classList.remove('hidden');
                cloudBtn.setAttribute('aria-expanded', 'true');
    ...
    // Close on outside click
    document.addEventListener('click', (e) => {
        if (cloudMenu.classList.contains('hidden')) return;
        if (cloudBtn.contains(e.target) || cloudMenu.contains(e.target)) return;
        cloudMenu.classList.add('hidden');
        cloudBtn.setAttribute('aria-expanded', 'false');
    });
    ...
    // Close on Escape key
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !cloudMenu.classList.contains('hidden')) {
            cloudMenu.classList.add('hidden');
            cloudBtn.setAttribute('aria-expanded', 'false');
        }
    });
...
// ---- Saves modal helpers (mobile only) ----
let _savesModalState = null;
function openSavesModal() {
    const forPurchases = (function isPurchasesActive() {
        const purchasesSec = document.getElementById('purchasesSection');
        return purchasesSec && !purchasesSec.classList.contains('hidden');
    })();

    const panel = document.getElementById(forPurchases ? 'purchaseSavePanel' : 'savePanel');
    if (!panel) return;

    // Ensure panel content is up to date (reuse existing logic)
    try {
        if (forPurchases) { if (typeof loadPurchaseSaves === 'function') loadPurchaseSaves(); if (typeof populatePurchaseSavePanel === 'function') populatePurchaseSavePanel(); }
        else { if (typeof loadSaves === 'function') loadSaves(); if (typeof populateSavePanel === 'function') populateSavePanel(); }
    } catch { /* no-op */ }

    // Create backdrop + centered shell
    const backdrop = document.createElement('div');
    backdrop.id = 'savesModalBackdrop';
    backdrop.className = 'fixed inset-0 z-50 bg-black/50 flex items-center justify-center';

    const shell = document.createElement('div');
    shell.id = 'savesModalShell';
    shell.className = 'w-full max-w-md px-3';
    ...
    // Neutralize dropdown positioning & size for modal
    ...
    // Mount and wire close interactions
    shell.appendChild(panel);
    document.body.appendChild(backdrop);

    const onKeydown = (e) => { if (e.key === 'Escape') closeSavesModal(); };
    backdrop.addEventListener('click', (e) => { if (e.target === backdrop) closeSavesModal(); });
    document.addEventListener('keydown', onKeydown);

    _savesModalState.onKeydown = onKeydown;
...
function closeSavesModal() {
    if (!_savesModalState) return;
    const { panel, parent, next, backdrop, onKeydown } = _savesModalState;

    // Restore styles and placement
    ...
    _savesModalState = null;
}

const mmSave = document.getElementById('mmSave');
// When triggered from hamburger: open as centered modal (do NOT toggle inline dropdown)
if (mmSave) mmSave.addEventListener('click', () => { closeMobileMenu(); openSavesModal(); });
...
function populateSavePanel() {
    ...
    tabs.className = 'flex items-center gap-2 justify-center mt-2';
    tabs.innerHTML = `
...
if (saveTransBtn) {
    saveTransBtn.addEventListener('click', () => {
        ...
        const method = transMethodEl ? transMethodEl.value.trim() : '';
...
function render() {
    ...
    // --- Update print-only balance message (Bootstrap-like alert) ---
    const pbm = document.getElementById('printBalanceMessage');
    if (pbm) {
        let cls = 'print-only alert ';
        let html = '';
        if (remaining > 0) {
            cls += 'alert-warning';
            html = `Balance due: <strong class="amount-emph">${fmt(remaining)}</strong>. Please arrange payment.`;
        } else if (remaining < 0) {
            cls += 'alert-info';
            html = `Overpayment credit: <strong class="amount-emph">${fmt(-remaining)}</strong> will be applied to future orders.`;
        } else {
            cls += 'alert-success';
            html = 'Thank you — account is fully settled.';
        }
        pbm.className = cls;
        pbm.innerHTML = html; // use HTML so the amount can be emphasized
    }
    ...
    if (itemBreakdown) {
        ...
        Object.entries(itemCounts).forEach(([itemName, count]) => {
            const div = document.createElement('div');
            // Tighter vertical rhythm for each row
            div.className = 'flex items-baseline justify-between py-px md:py-0.5';
        ...
        // Footer row: Total pieces — slightly tighter gap
        const totalPieces = Object.values(itemCounts).reduce((sum, n) => sum + n, 0);
        const footer = document.createElement('div');
        footer.className = 'mt-0.5 flex items-baseline justify-between border-t pt-1';
        footer.innerHTML = `
        <span class="text-neutral-500">Total pieces</span>
        <span class="text-emerald-600 font-semibold text-2xl tabular-nums">${totalPieces}</span>
    `;
        itemBreakdown.appendChild(footer);
    }
...
// Build the transactions card: totals + professional table (Date, Type, Amount, Note, Action)
function renderTransactions() {
    if (!transactionsTotalsEl || !transactionsListEl) return;

    // Compute totals for payments and deductions
    let autoPaid = 0;
    for (const r of rows) autoPaid += r.paidCents || 0;

    let manualPaid = 0, manualDeduct = 0;
    for (const t of transactions) {
    ...
    // Update totals display: compact grid with a subtle vertical divider & refined spacing
    transactionsTotalsEl.innerHTML = `
    <div class="inline-grid grid-cols-[auto_auto] items-baseline gap-x-2 sm:gap-x-3">
        <div class="space-y-1.5 leading-6">
        <div>Total Paid</div>
        <div>Total Deductions</div>
        </div>
        <div class="pl-3 sm:pl-4 border-l border-neutral-300/40 dark:border-neutral-600/40 space-y-1.5 leading-6">
        <div class="font-medium tabular-nums">${fmt(totalPaid)}</div>
        <div class="font-medium tabular-nums">${fmt(totalDeduct)}</div>
        </div>
    </div>
    `;

    // Combine auto + manual (>0) and sort by date desc
    ...
    // Render table rows into <tbody id="transactionsTbody">
    ...
    for (const t of all) {
        ...
        const isManual = !String(t.id).startsWith('auto-');
        const typeLabel = t.type === 'payment' ? 'Payment' : 'Deduction';
        const typeClass = t.type === 'payment' ? 'pill pill--pay' : 'pill pill--ded';
        const notesEsc = (t.notes || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const methodEsc = (t.method || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

        // attach id for editing lookups
        tr.dataset.tid = t.id;

        tr.innerHTML = `
<td class="py-2 pr-3 whitespace-nowrap ${isManual ? 'editable txn-edit txn-date' : 'txn-date'}"
    data-field="date" title="${isManual ? 'Click to edit' : ''}">${fmtDate(t.date)}</td>
<td class="py-2 pr-3 ${isManual ? 'editable txn-edit' : ''}"
    data-field="type" title="${isManual ? 'Click to edit' : ''}"><span class="${typeClass}">${typeLabel}</span></td>
<td class="py-2 pr-3 ${isManual ? 'editable txn-edit' : ''}"
    data-field="method" title="${isManual ? 'Click to edit' : ''}">${methodEsc}</td>
<td class="py-2 pr-3 text-right tabular-nums ${isManual ? 'editable txn-edit txn-amount' : 'txn-amount'}"
    data-field="amountCents" title="${isManual ? 'Click to edit' : ''}">${fmt(t.amountCents)}</td>
<td class="py-2 pr-3 max-w-[360px] truncate ${isManual ? 'editable txn-edit txn-note' : 'txn-note'}"
    data-field="notes" title="${isManual ? 'Click to edit' : ''}">${notesEsc}</td>
<td class="py-2 pr-0 text-right print-hide">
  ${isManual ? `<button class="rounded-lg border px-2 py-1 text-red-600 hover:bg-red-50 transition-base txn-del">Delete</button>` : ''}
</td>
`;

        transactionsListEl.appendChild(tr);

        // Toggle the "No transactions yet" placeholder row if present
        const emptyRow = document.getElementById('transactionsEmptyRow');
        if (emptyRow) {
            emptyRow.classList.toggle('hidden', transactionsListEl.children.length > 0);
        }

        // Attach delete for manual entries
        if (isManual) {
            const btn = tr.querySelector('.txn-del');
            if (btn) {
                btn.addEventListener('click', () => {
                    showConfirmModal(
    ...
    // Show the placeholder row when no transactions
    const emptyRowEl = document.getElementById('transactionsEmptyRow');
    if (emptyRowEl) emptyRowEl.classList.toggle('hidden', all.length > 0);
}
...
// Build a clean summary table and clone the item breakdown below it.
// NOTE: Do NOT inject the balance message here anymore — it now lives
// under Current Balance inside the Total value card for print.
...
// === Navbar active visuals for menus ===
(function enhanceNavbarActiveStates() {
    // helper to mark a button active based on expanded or panel visibility
    function bindExpandedActive(btn, menu) {
        if (!(btn && menu)) return;
        const set = () => {
        ...
        // also when menu visibility toggles
        const obs2 = new MutationObserver(() => {
            const open = !menu.classList.contains('hidden');
            btn.classList.toggle('is-active', open);
        });
        obs2.observe(menu, { attributes: true, attributeFilter: ['class'] });
    }
    // more menu
    bindExpandedActive(document.getElementById('navMore'), document.getElementById('moreMenu'));
    // cloud menu
    bindExpandedActive(document.getElementById('cloudSyncBtn'), document.getElementById('cloudSyncMenu'));
    // save panel (two panels: deliveries & purchases)
    (function () {
        const saveBtn = document.getElementById('navSave');
        const p1 = document.getElementById('savePanel');
        const p2 = document.getElementById('purchaseSavePanel');
        if (!saveBtn) return;
        const update = () => {
            const open = (p1 && !p1.classList.contains('hidden')) || (p2 && !p2.classList.contains('hidden'));
            saveBtn.classList.toggle('is-active', !!open);
        };
        // Observe both panels
        if (p1) {
            new MutationObserver(update).observe(p1, { attributes: true, attributeFilter: ['class'] });
        }
        if (p2) {
            new MutationObserver(update).observe(p2, { attributes: true, attributeFilter: ['class'] });
        }
    })();
})();

// === Print helpers: hide Transactions on print if totals are all $0.00, and ensure print summary is fresh ===

// Safely get the Transactions card container
function getTransactionsCardEl() {
    const byId = document.getElementById('transactionsCard');
    if (byId) return byId;
    const table = document.getElementById('transactionsTable');
    return table ? table.closest('.bg-white') : null;
}

// Determine whether Transactions should be hidden on print
function shouldHideTransactionsInPrint() {
    // Auto-paid from deliveries
    let autoPaid = 0;
    for (const r of rows) autoPaid += r.paidCents || 0;

    // Manual transactions (payments/deductions)
    let manualPaid = 0, manualDeduct = 0;
    for (const t of transactions) {
        if ((t.amountCents || 0) <= 0) continue;
        if (t.type === 'payment') manualPaid += t.amountCents || 0;
        else manualDeduct += t.amountCents || 0;
    }
    ...
    const totalPaid = autoPaid + manualPaid;
    const totalDeduct = manualDeduct;
    return totalPaid === 0 && totalDeduct === 0;
}

function applyPrintTransactionsVisibility() {
    const card = getTransactionsCardEl();
    if (!card) return;
    if (shouldHideTransactionsInPrint()) {
        card.classList.add('print-hide');   // only hides in @media print
    } else {
        card.classList.remove('print-hide');
    }
}

// Keep print summary + item breakdown current, and toggle TXN visibility right before printing
window.addEventListener('beforeprint', () => {
    if (typeof updatePrintSummaryAndBreakdown === 'function') {
        updatePrintSummaryAndBreakdown();
    }
    applyPrintTransactionsVisibility();
});

// Optional: tidy up after printing (not required, since .print-hide is scoped to @media print)
