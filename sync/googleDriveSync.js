...
(() => {
    ...
    const dot = $('cloudStatusDot');   // small visual state indicator
    const passI = $('cloudPassphrase');

    if (!btn || !menu || !chip || !dot) { console.warn('[GDSync] UI not found'); return; }

    // ====== Tooltip (for confirmations) ======
    let tip, tipTimer;
    function ensureTip() {
        if (tip) return tip;
    ...
    function showTip(text, state = 'ok', ms = 2200) {
        ...
        tip.style.visibility = 'hidden'; // wait for size
    ...
    // Update sr-only live region + dot color
    function setState(state, text) {
        chip.textContent = text; // announce for screen readers
        dot.classList.remove('cloud-dot-muted', 'cloud-dot-sync', 'cloud-dot-ok', 'cloud-dot-off', 'cloud-dot-err');
        dot.classList.add({
            muted: 'cloud-dot-muted',
            sync: 'cloud-dot-sync',
            ok: 'cloud-dot-ok',
            off: 'cloud-dot-off',
            err: 'cloud-dot-err'
        }[state] || 'cloud-dot-muted');
    }
    function flashState(state, text, revertMs = 2500) {
        setState(state, text);
        showTip(text, state, revertMs);
        // Revert to "Up to date" unless we are offline or need sign-in
        if (state !== 'off' && state !== 'muted') {
    ...
    async function syncNow() {
        ...
        if (!shouldSync()) { flashState('muted', 'Nothing to sync'); return; }
    ...
    window.addEventListener('focus', () => { if (shouldSync()) syncNow().catch(() => { }); }); // focus-sync ON
    ...
    (function boot() {
        try {
            ...
            else setState('muted', 'Needs sign-in');
        } catch { setState('muted', 'Needs sign-in'); }
        refreshMenuState();
        // No distracting toasts on first load.
