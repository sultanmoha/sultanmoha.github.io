...
(() => {
    ...
    const PAUSE_KEY = 'gdsync.paused';   // "true" | "false"
    const LASTOK_KEY = 'gdsync.lastOkAt'; // timestamp of last successful sync

    // ====== DOM ======
    const $ = (id) => document.getElementById(id);
    const btn = $('cloudSyncBtn');
    const menu = $('cloudSyncMenu');
    const chip = $('cloudStatusChip');    // sr-only live region
    const mainI = $('cloudMainIcon');      // main cloud icon
    const badge = $('cloudStatusBadge');
    const bIcon = $('cloudBadgeIcon');
    const passI = $('cloudPassphrase');

    if (!btn || !menu || !chip || !mainI || !badge || !bIcon) { console.warn('[GDSync] UI not found'); return; }
    ...
    function positionTip() {
        ...
        // place the tooltip UNDER the button with an 8px gap
        tip.style.top = `${r.bottom + 8}px`;
    }
    function showTip(text, state = 'ok', ms = 2200) {
        ensureTip();
        tip.className = `cloud-tip cloud-tip-${state}`;
    ...
    // --- NEW: menu status helpers ---
    let _lastOkAt = parseInt(localStorage.getItem(LASTOK_KEY) || '0', 10);
    ...
    function updateMenuStatusLine() {
        const el = document.getElementById('cloudMenuStatus');
        if (!el) return;
        el.classList.remove('status-success', 'status-sync', 'status-paused', 'status-error', 'status-muted', 'status-offline');

        switch (currentState) {
            case 'sync':
                el.textContent = 'Syncing…';
                el.classList.add('status-sync');
                break;
            case 'paused':
                el.textContent = 'Sync paused';
            ...
            default: { // ok
                if (_lastOkAt > 0) {
                    el.textContent = `Synced ${timeAgo(now() - _lastOkAt)}`;
    ...
    // refresh the “x min ago” text while menu is open/closed
    setInterval(updateMenuStatusLine, 60_000);

    const isPaused = () => localStorage.getItem(PAUSE_KEY) === 'true';
    function setPaused(v) { localStorage.setItem(PAUSE_KEY, v ? 'true' : 'false'); refreshMenuState(); updateBadgeForState(currentState); }

    // ====== Status handling (badge + a11y text + main icon tint) ======
    // states: 'ok' | 'sync' | 'paused' | 'off' (offline) | 'blocked' (needs sign-in) | 'err'
    let currentState = 'ok';
    function setState(state, text) {
        ...
        // main cloud tint
        btn.classList.toggle('cloud-main-off', state === 'off');
        btn.classList.toggle('cloud-main-paused', state === 'paused');

        // badge reset
        badge.classList.remove('cloud-badge-ok', 'cloud-badge-sync', 'cloud-badge-paused', 'cloud-badge-off', 'cloud-badge-err', 'cloud-badge-spin', 'cloud-badge-beat');
        bIcon.className = 'fa-solid'; // reset

        switch (state) {
            case 'sync':
                badge.classList.add('cloud-badge-sync', 'cloud-badge-spin');
                bIcon.classList.add('fa-rotate');
                break;
            case 'paused':
                badge.classList.add('cloud-badge-paused', 'cloud-badge-beat');
                bIcon.classList.add('fa-pause');
                break;
            case 'off': // offline → red minus
                badge.classList.add('cloud-badge-off', 'cloud-badge-beat');
                bIcon.classList.add('fa-minus');
                break;
            case 'blocked': // not signed in
                badge.classList.add('cloud-badge-off', 'cloud-badge-beat');
                bIcon.classList.add('fa-minus');
                break;
            case 'err':
                badge.classList.add('cloud-badge-err', 'cloud-badge-beat');
                bIcon.classList.add('fa-xmark');
                break;
            default: // ok
                badge.classList.add('cloud-badge-ok');
                bIcon.classList.add('fa-check');
                // record last successful sync time
                _lastOkAt = now();
                localStorage.setItem(LASTOK_KEY, String(_lastOkAt));
                break;
        }

        // keep the menu status line in sync with the state
        updateMenuStatusLine();
    }
    function flashState(state, text, revertMs = 2200) {
        setState(state, text);
        showTip(text, state === 'ok' ? 'ok' : (state === 'sync' ? 'sync' : (state === 'off' ? 'off' : (state === 'err' ? 'err' : 'muted'))), revertMs);
        if (state !== 'off' && state !== 'blocked' && state !== 'paused') {
            setTimeout(() => setState('ok', 'Up to date'), revertMs);
        }
    }
    function updateBadgeForState(state) {
        // if paused request, enforce paused visuals regardless of last state
        if (isPaused()) {
            setState('paused', 'Sync paused');
            return;
        }
        setState(state, chip.textContent || ''); // keep current text
    }
    ...
    function ensurePauseMenuItem() {
        // Insert Pause/Resume after the "Sync now" item
        let pauseItem = menu.querySelector('.cloud-item[data-action="pause"]');
        if (!pauseItem) {
            const after = menu.querySelector('.cloud-item[data-action="sync"]');
            pauseItem = document.createElement('button');
            pauseItem.type = 'button';
            pauseItem.className = 'cloud-item';
            pauseItem.setAttribute('data-action', 'pause');
            if (after && after.parentNode) after.parentNode.insertBefore(pauseItem, after.nextSibling);
            else menu.insertBefore(pauseItem, menu.firstChild);
        }
        pauseItem.innerHTML = isPaused()
            ? `<i class="fa-solid fa-play" aria-hidden="true"></i> Resume sync`
            : `<i class="fa-solid fa-pause" aria-hidden="true"></i> Pause sync`;
    }
    ...
    function refreshMenuState() {
        ...
        setDis(q('sync'), !signedIn || paused);
        setDis(q('restore'), !signedIn);
        setDis(q('view'), !signedIn);
        setDis(q('signout'), !signedIn);

        if (!signedIn) {
            setState('blocked', 'Needs sign-in');
            const pauseItem = q('pause'); if (pauseItem) setDis(pauseItem, true);
        } else {
            const pauseItem = q('pause'); if (pauseItem) setDis(pauseItem, false);
        }
    }
    ...
    function showMenu(show) {
        if (show) {
            ...
            const panel = menu.querySelector('#cloudBackupsPanel');
            if (panel) panel.classList.add('hidden');
        }
        refreshMenuState();
        updateMenuStatusLine(); // NEW: ensure the status line reflects the latest state/time
    }
    ...
    async function ensureToken(interactive = false) {
        if (navigator.onLine === false) { setState('off', 'Offline'); queued = true; throw new Error('Offline'); }
    ...
    function applyLocalSnapshot(snap) {
        ...
        // Reload in-memory state & UI (mirror your boot/import sequence)
        try {
    ...
    // Only data keys trigger autosync (ignore view/tab switches)
    ...
    async function syncNow() {
        ...
        if (isPaused()) { flashState('paused', 'Sync paused'); return; }
        if (!shouldSync()) { flashState('blocked', 'Nothing to sync'); return; }

        isSyncing = true; setState('sync', 'Syncing…');
        try {
            ...
            flashState('ok', 'Synced');
            queued = false;
        } catch (e) {
            if (navigator.onLine === false) { queued = true; flashState('off', 'Offline'); }
            else { flashState('err', 'Error'); }
        } finally {
            isSyncing = false;
        }
    }
    ...
    function scheduleAutosync(changedKey) {
        if (queued || isPaused()) return;
        if (!shouldSync()) return;
        if (changedKey && !SYNC_KEYS.has(changedKey)) return;
    ...
    menu.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (action === 'pause') {
                ...
                setPaused(nowPaused);
                flashState(nowPaused ? 'paused' : 'ok', nowPaused ? 'Sync paused' : 'Sync resumed');
                ensurePauseMenuItem();  // update label
                return;
            }
            if (action === 'restore') {
                if (window.showConfirmModal) {
        ...
        `;
            ...
            flashState('err', 'Error');
        }
    });

    // ====== network/tab events ======
    window.addEventListener('online', () => { if (queued && shouldSync() && !isPaused()) syncNow().catch(() => { }); });
    window.addEventListener('focus', () => { if (shouldSync() && !isPaused()) syncNow().catch(() => { }); }); // focus-sync ON

    // ====== expose API ======
    window.GDSync = { signIn: () => ensureToken(true), signOut: revokeToken, isSignedIn: () => !!accessToken, syncNow, restoreLatest, listBackups, pause: () => setPaused(true), resume: () => setPaused(false), isPaused };

    // ====== boot ======
    (function boot() {
        try {
            const cached = JSON.parse(sessionStorage.getItem(TOKEN_KEY) || 'null');
            if (cached?.accessToken) { accessToken = cached.accessToken; tokenExpireAt = cached.tokenExpireAt || 0; setState('ok', 'Signed'); }
            else setState('blocked', 'Needs sign-in');
        } catch { setState('blocked', 'Needs sign-in'); }
        if (isPaused()) setState('paused', 'Sync paused');
        refreshMenuState();
